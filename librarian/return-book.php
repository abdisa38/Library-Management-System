...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && validateCSRFToken($_POST['csrf_token'])) {
    try {
        ...
        if (isset($_POST['borrow_id'])) {
            ...
            // Get borrow record with student info
            $stmt = $db->prepare("SELECT br.*, s.user_id, b.book_title 
            ...
            if (!$borrow) {
                ...
                if ($borrow['user_id']) {
                    $message = "Your book '{$borrow['book_title']}' has been returned successfully.";
                    if ($fine > 0) {
                        $message .= " Fine: " . formatCurrency($fine);
                    }
                    sendNotification($borrow['user_id'], $message);
                }
        ...
        if (isset($_POST['send_reminder'])) {
            $borrowId = $_POST['reminder_borrow_id'];
            
            $stmt = $db->prepare("SELECT br.*, s.user_id, s.full_name, b.book_title, DATEDIFF(CURDATE(), br.due_date) as days_overdue
                                  FROM borrow_records br
                                  INNER JOIN students s ON br.student_id = s.id
                                  INNER JOIN books b ON br.book_id = b.id
                                  WHERE br.id = ?");
            ...
            if ($record && $record['user_id']) {
                $message = "REMINDER: Your borrowed book '{$record['book_title']}' is overdue by {$record['days_overdue']} days. Please return it as soon as possible to avoid additional fines.";
                sendNotification($record['user_id'], $message);
                
                setSuccessMessage('Reminder sent to ' . $record['full_name']);
                redirect(SITE_URL . '/librarian/return-book.php');
            } else {
                setErrorMessage('Unable to send reminder');
            }
        }
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <form method="POST" style="display: inline;">
                                <input type="hidden" name="csrf_token" value="<?php echo generateCSRFToken(); ?>">
                                <input type="hidden" name="reminder_borrow_id" value="<?php echo $borrow['id']; ?>">
